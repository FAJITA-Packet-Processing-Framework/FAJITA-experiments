...
### Input file
baseline_file = "OFRXPPS.csv"
llc_file = "LLC-MISSES-PP.csv"
y_scale=1
...
plot baseline_file using ($1*24/1000000):($5):($4):($6) with errorbars ls 1 title "Throughput",\
     baseline_file using ($1*24/1000000):($5) with lines ls 1 dt 2 notitle,\
     llc_file using ($1*24/1000000):($5 * 8):($3 * 8):($7 * 8) with errorbars ls 3 title "LLC misses" axis x1y2,\
     llc_file using ($1*24/1000000):($5 * 8) with lines ls 3 dt 2 notitle axis x1y2
unset multiplot
