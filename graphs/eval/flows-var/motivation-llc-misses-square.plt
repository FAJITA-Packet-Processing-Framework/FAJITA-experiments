...
### Input files
input_file_4_ways  = "fajita-csvs/FCLLC-MISSES-PP.csv"
input_file_12_ways  = "fajita-csvs/FAJITALLC-MISSES-PP.csv"
...
# X-axis
set xlabel "Total Number of Flows (m)"
...
plot \
        input_file_12_ways  using ($1*24/1000000):($5 * 8):($3 * 8):($7 * 8) with errorbars ls 1 title "FAJITA",\
        input_file_12_ways  using ($1*24/1000000):($5 * 8) with lines ls 1 dt 5 notitle,\
        input_file_4_ways  using ($1*24/1000000):($5 * 8):($3 * 8):($7 * 8) with errorbars ls 4 title "FastClick+BR",\
        input_file_4_ways  using ($1*24/1000000):($5 * 8) with lines ls 4 dt 5 notitle
unset multiplot
