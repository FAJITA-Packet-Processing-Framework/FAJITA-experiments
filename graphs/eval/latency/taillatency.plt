...
set output "eval-tail-latency.pdf"
...
### Input files
input_file_1_way = "csvs/FCLAT99.csv"
input_file_2_ways  = "csvs/FAJITALAT99.csv"
...
# Y-axis
set ylabel "{/Helvetica:Bold 99th percentile Latency ({/Symbol m}s)}"
...
plot \
	input_file_2_ways  using ($1*6/1000000):($5):($2):($8) with errorbars ls 1 title "FAJITA",\
        input_file_2_ways  using ($1*6/1000000):($5) with lines ls 1 dt 5 notitle,\
        input_file_1_way  using ($1*6/1000000):($5):($2):($8) with errorbars ls 4 title "FastClick+BR",\
